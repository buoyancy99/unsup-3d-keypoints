...
class MJCFBasedRobot(XmlBasedRobot):
    ...
    def reset(self, bullet_client):
        ...
        if (self.doneLoading == 0):
            self.ordered_joints = []
            self.doneLoading = 1
            path = os.path.join(pybullet_data.getDataPath(), "mjcf", self.model_xml.replace('_nocolor', ''))
            if self.self_collision:
                self.objects = self._p.loadMJCF(path,
        ...
        if self.model_xml == 'ant_nocolor.xml':
            for i in range(-1, 20):
                p.changeVisualShape(1, i, rgbaColor=[0.8, 0.6, 0.4, 1])
